...
- Derived content: generate tags, frontmatter metadata, preview text, word count, custom property pills, and feature images using background content providers.

The cache includes:

- File mtimes plus provider-specific processed mtimes
- Tags, frontmatter-derived metadata, word count, custom property pills
...
- Provider outputs:
  ...
  - `customProperty`: resolved custom property pill list (`null` until generated; markdown only)

Defaults are set by `createDefaultFileData()` (`src/storage/IndexedDBStorage.ts`):

- Markdown: `tags=null`, `metadata=null`, `wordCount=null`, `customProperty=null`, `previewStatus='unprocessed'`
- PDF: `tags=[]`, `metadata={}`, `wordCount=0`, `customProperty=null`, `previewStatus='none'`
...
- Content providers (`src/services/content/*`):
  - `MarkdownPipelineContentProvider` (`markdownPipeline`): preview, word count, custom property pills, markdown feature images
...
- Providers decide whether a file needs work by comparing their processed mtime field (`*Mtime`) to `file.stat.mtime` and checking status fields (`previewStatus`, `featureImageStatus`, `featureImageKey`, `tags`, `metadata`, `wordCount`, `customProperty`).
...
- `buildFileCache(true)` (initial-load path in `useStorageVaultSync`) seeds the database by diffing against an empty cache:
  - New markdown records start with `tags=null`, `metadata=null`, `wordCount=null`, `customProperty=null`, and `previewStatus='unprocessed'`.
  - New PDF records start with `tags=[]`, `metadata={}`, `wordCount=0`, `customProperty=null`, and `previewStatus='none'`.
...
- Storage is ready when the initial diff + tag tree build completes and `StorageContext` sets `isStorageReady=true` (`useStorageVaultSync`).
- A rebuild is complete when the rebuild notice sees no remaining pending work across the tracked content types (`useCacheRebuildNotice`):
  ...
  - Custom property: markdown files where `customProperty === null` (when custom property tracking is enabled)
