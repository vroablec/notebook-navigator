    ...
    navigationCalendar: {
        ...
        helpModal: {
            ...
            items: [
                ...
                '`Cmd/Ctrl+Click` по дате, чтобы добавить её как фильтр поиска. `Option/Alt+Click` также работает, если модификатор опции включён в настройках.'
            ]
        }
    },
            ...
            sections: {
                ...
                connectors: {
                    ...
                    items: [
                        ...
                        'Если запрос включает имена, даты (`@...`) или фильтры задач (`has:task`), `AND` и `OR` ищутся как слова.',
                        'Пример запроса с операторами: `#work OR #home`.',
                        'Пример смешанного запроса: `#work OR has:task` (`OR` ищется в именах файлов).'
                    ]
                },
                dates: {
                    title: 'Даты',
