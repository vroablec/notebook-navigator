...
export const STRINGS_UK = {
    ...
    navigationCalendar: {
        ...
        helpModal: {
            ...
            items: [
                ...
                '`Cmd/Ctrl+Click` на даті, щоб додати її як фільтр пошуку. `Option/Alt+Click` також працює, коли модифікатор опції увімкнено в налаштуваннях.'
            ]
        }
    },
    ...
    searchInput: {
        ...
        searchHelpModal: {
            intro: 'Комбінуйте імена файлів, теги та дати в одному запиті (напр. `meeting #work @thisweek`). Встановіть плагін Omnisearch для повнотекстового пошуку.',
            introSwitching:
            ...
            sections: {
                ...
                connectors: {
                    ...
                    items: [
                        '`AND` та `OR` є операторами лише в запитах, що складаються виключно з тегів.',
                        'Запити з тегів містять лише фільтри тегів: `#tag`, `-#tag`, `#`, `-#`.',
                        'Якщо запит включає імена, дати (`@...`) або фільтри завдань (`has:task`), `AND` та `OR` шукаються як слова.',
                        'Приклад запиту з операторами: `#work OR #home`.',
                        'Приклад змішаного запиту: `#work OR has:task` (`OR` шукається в іменах файлів).'
                    ]
                },
                dates: {
                    title: 'Дати',
